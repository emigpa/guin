...
export default new Vuex.Store({
  mutations: {
    setSigevaData (state, data) {
      state.sigevadata = data
      state.loaded = true
    }
  },
  actions: {
    getSigevaData ({ commit }, query) {
      axios.get(`http://localhost:8080/api/guin`, {params: { cuil: query }})
      .then(res => {
        ...
        commit('setSigevaData', sigevadata)
      })
    }
  }
